...
echo ""; echo ""; echo "When you issue 'bbl up' to deploy BOSH on $BBL_IAAS, your state will be stored in $BBL_STATE_DIRECTORY"
echo "Investigate \"bbl -h\" AND \"bbl up -h\" for appropriate command line options to leverage."
echo "My favorites are \"bbl up --debug --lb-type concourse\"", but your mileage may vary.
echo 'After "bbl up" completes, do an eval "$(bbl print-env)" **with quotes** to export BOSH environment variables appropriately'
